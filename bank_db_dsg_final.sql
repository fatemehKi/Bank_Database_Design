/**
 * @file Project.sql *
 * @The goal of the project is to create bank data base
...
/********QUESTION 1**************/
/********Create a database for a banking application called “Bank”. [Basic]****/
create database bank
use bank

/********QUESTION 2**************/
/********Create all the tables mentioned in the database diagram. [Moderate]****/
...
/***dropping tables in case it is required**/
drop table
Login_account, 
UserLogins, 
UserSecurityAnswers,
...
/********QUESTION 3**************/
/********Create all the constraints based on the database diagram. [Advanced]****/
/**foreign key creation
template:
Alter table child add constraint constrain_name 
foreign key (name in the child table) references parent(name in parent tale)
*************/
...
/***dropping FKs if it is required**/
...
/********QUESTION 4**************/
/********Insert at least 5 rows in each table. [Basic]****/
...
/***checking tables in case it is required****/
select * from 
Login_account, 
UserLogins, 
UserSecurityAnswers,
...
/********QUESTION 1**************/
/********Create a view to get all customers with checking account from ON province. [Moderate]****/
...
/********QUESTION 2**************/
/********Create a view to get all customers with total account balance (including interest rate) greater than 5000. [Advanced]****/
CREATE VIEW VA_high_balance_Cu
AS
SELECT CustomerID, CurrentBalance, concat(CustomerFirstName, ' ', CustomerMiddleInitail,' ', CustomerLastName) as Customer_Name from customer 
join Account
...
select * from VA_high_balance_Cu

/********QUESTION 3**************/
/********Create a view to get counts of checking and savings accounts by customer. [Moderate]****/
...
--second response in case the meaing of question is total number of save+checking
Create VIEW VA_SaChe_Cnt2
AS
...
/********QUESTION 5**************/
/********Create a view to get all customers’ overdraft amount. [Moderate]*/
...
/********QUESTION 6**************/
/********Create a stored procedure to add “User_” as a prefix to everyone’s login (username). [Moderate]*/
...
/********QUESTION 7**************/
/********Create a stored procedure that accepts AccountId as a parameter and returns customer’s full name. [Advanced]*/
...
/********QUESTION 8**************/
/********Create a stored procedure that returns error logs inserted in the last 24 hours. [Advanced]*/
...
/********QUESTION 9**************/
/********Create a stored procedure that takes a deposit as a parameter and updates CurrentBalance value for that particular account. [Advanced]*/
...
/********QUESTION 10**************/
/********Create a stored procedure that takes a withdrawal amount as a parameter and updates CurrentBalance value for that particular account. [Advanced]*/
...
/********QUESTION 11**************/
/********Create a stored procedure to remove all security questions for a particular login. [Moderate]*/
create procedure remove_se_Q @userLoginID smallint
as 
delete UserSecurityQuestions
from UserSecurityAnswers join UserSecurityQuestions
on UserSecurityAnswers.UserSecurityQuestionID = UserSecurityQuestions.UserSecurityQuestionID
go
select * from UserSecurityQuestions

/**we need to drop fk first*/
exec remove_se_Q 101
